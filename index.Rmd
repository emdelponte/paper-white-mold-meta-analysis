...
Table. Significance of the test of whether the moderator accounts for any of the heterogeneity in these data  explained variability by the inclusion of moderators and the amount of variance (heterogeneity) accounted for by the moderator (R2 statistics).
```{r}
...
dat_yld3 %>% 
 ...
 # geom_hline(yintercept = ma_cor_yld$pred, size=0.75)+
  #geom_hline(yintercept = c(ma_cor_yld$cr.lb, ma_cor_yld$cr.ub), linetype="dashed")+ 
  coord_flip()+
...
dat_scl3 %>% 
  ...
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), color="grey50") + 
  geom_point(aes(size = 1/vi), shape = 15, color="grey70") +
  #geom_hline(yintercept = ma_cor_scl$pred, size=0.75)+
  #geom_hline(yintercept = c(ma_cor_scl$ci.lb, ma_cor_scl$ci.ub), linetype="dashed")+ 
  coord_flip()+
  labs(x = "Slope") + 
  theme_minimal() + 
  labs(size = "1/vi", 
